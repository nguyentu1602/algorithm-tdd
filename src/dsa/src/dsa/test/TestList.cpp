...
// this function print a Vector of Lists named arr
...
void print( const Vector<List<int>> arr )
{
    ...
    {
        cout << "arr[" << i << "]:";
        // idiom: range-based for loop. Equal to "for each item"
        // for each item x in the list arr [i]
        for( auto x : arr[ i ] )
            cout << " " << x;
        cout << endl;
    }
...
{
    const int N = 20;
    Vector<List<int>> arr( N ); // init a vector that contains 20 lists of ints
    List<int> lst;
